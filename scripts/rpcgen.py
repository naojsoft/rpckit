...
#
# BUGS:
# `typedef stuct/enum/union {...} id;` does not work, must use `stuct id {...};` form
#
# union arm-declarations can only be a single line
#
# no checking is done on use of negative numbers
#
...
reserved = ("INT", "SHORT", "BOOL", "CASE", "CONST", "DEFAULT", "DOUBLE", "QUADRUPLE",
...
class RPCType:
    ...
    def __init__(self, base_type=None,
        ...
        # FRED note this is a string, possibly a constant name
        self.arraylen = arraylen
        # 1 if this is an true array. Note: string and opaque type
        # does not count as arrays, but they uses vararray, fixarray
        # and arraylen fields.
        # FRED - despite above comment, gen_packers requires this to be set
        # even if it is a string or opaque
    ...
    def array_string(self):
        if self.arraylen:
            lenstring = str(self.arraylen).replace(constants_file+'.','')
        else:
            lenstring = ""
...
def gen_pack_code(ip, id, typedecl, classname):
    ...
    if typedecl.isarray:
        if typedecl.base_type not in ["opaque", "string"]:
            name = "array"
            extra = ", self.pack_%s" % typedecl.base_type
        else:
            name = typedecl.base_type
            extra = ""
...
def gen_unpack_code(ip, id, typedecl, classname):
    ...
    if typedecl.isarray:
        if typedecl.base_type not in ["opaque", "string"]:
            name = "array"
            extra = ", self.unpack_%s" % typedecl.base_type
        else:
            name = typedecl.base_type
            extra = ""
        if typedecl.fixarray:
            ip.pr('data.%s = self.unpack_f%s(%s%s)' % (id, name, typedecl.arraylen, extra))
        else:
...
def gen_packers(id, typeobj):
    ...
    if not base_type:
        print("""Warning: definition of '%s' is based on '%s' which isn't defined yet. Assuming '%s' is composite (struct/union).""" \
              % (id, typeobj.base_type, typeobj.base_type), file=sys.stderr)
    ...
    if typeobj.isarray:
        ...
        ip.change(4)
        if typeobj.base_type not in ["opaque", "string"]:
            name = "array"
            extra = ", self.pack_%s" % typeobj.base_type
        else:
            name = typeobj.base_type
            extra = ""
        if typeobj.fixarray:
        ...
        # Simple alias
        ip.pr("pack_%s = pack_%s" % (id, typeobj.base_type))
    ip.cont('\n')

    # Unpackers
    ip = IndentPrinter(unpacker_out)
    ip.change(4)
    if not typeobj.isarray:
        # Simple alias
        ...
        if typeobj.base_type not in ["opaque", "string"]:
            name = "array"
            extra = ", self.unpack_%s" % typeobj.base_type
        else:
            name = typeobj.base_type
            extra = ""
        if typeobj.fixarray:
            ip.pr('return self.unpack_f%s(%s%s)' % (name, typeobj.arraylen, extra))
        else:
            if typeobj.arraylen:
...
def gen_switch_code(ip, union_body, packer, classname, assertions=0):
    ...
    for case_declaration in [first_case_declaration] + case_list:
        ...
        if not value.isdigit():
            value = "%s.%s" % (constants_file, value)
        declaration = case_declaration.declaration

        check_not_reserved(switch_id, value)
        if virgin:
            ip.change(-4)
            ip.prcomma("if data.%s == %s" % (switch_id, value))
            virgin = 0
        elif not last_empty:
            ip.change(-4)
            ip.prcomma("elif data.%s == %s" % (switch_id, value))
        else:
            # Last turn had empty declaration.
            ip.cont(" or data.%s == %s" % (switch_id, value))
    ...
    if default_declaration:
        ...
        ip.pr('raise %s.BadDiscriminant(data.%s, data)' % (types_file, switch_id))
    ip.change(-4)
...
#   print "Creating class for struct", t[2]
    ...
    ip = IndentPrinter(packer_out)
    ip.change(4)
    ip.pr('def pack_%s(self, data):' % classname)
    ip.change(4)
    # assert_not_none
    ip.pr('if not isinstance(data, %s.%s):' % (types_file, classname))
    ip.pr('    raise TypeError, "pack_%s"' % classname)
    ...
    ip = IndentPrinter(unpacker_out)
    ip.change(4)
    ip.pr("def unpack_%s(self):" % classname)
    ip.change(4)
    ip.pr('data = %s.%s()' % (types_file, classname))
    for (id, typedecl) in struct_body:
...
def p_constant_def(t):
    ...
    print(t[2], t[3], t[4], file=const_out)
...
def p_enum_constant(t):
    ...
    print(t[1], t[2], t[3], file=const_out)
...
def p_type_specifier_1(t):
    ...
    if t[1] == None:
        ...
        raise ValueError("invalid keyword prepending int or hyper")
...
def p_declaration_2(t):
    ...
    id = t[2]
    arraylen = t[4]
    if arraylen and not arraylen.isdigit():
        arraylen = "%s.%s" % (constants_file, arraylen)
...
def p_version_def(t):
    ...
    id = t[2]
    version = t[8]
    print("%s = %s" % (id, version), file=const_out)
...
def p_procedure_def(t):
    ...
    id = t[2]
    proc_num = t[8];
    print("%s = %s" % (id, proc_num), file=const_out)
...
const_out = None
types_out = None
packer_file_out = None
packer_out = StringIO()
unpacker_out = StringIO()
