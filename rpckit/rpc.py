...
MAXSEQ = 0x80000000  # Maximum gss sequence number value
        ...
        if flavor in [AUTH_NULL, AUTH_UNIX]:
            ...
            while True:
                ...
                if major not in good_major:
                    raise RPCSecurity("Bad return value from initSecContext: %i" % major)
                ...
                if token and (reply_major != gssapi.GSS_S_COMPLETE):
                    ...
                    if reply_major not in good_major:
                        raise RPCSecurity("Bad return value from server: %i" % reply_major)
                    init = 2
                if major == gssapi.GSS_S_COMPLETE:
...
class Server:
    ...
    def handle_threaded(self, call, packer, unpacker):
        ...
        methname = 'handle_threaded_' + repr(proc)
        try:
...
class AsyncTCPServer(Server):
    ...
    def asyncread(self, sock):
        ...
        if len(self.read_buffers[sock]) > 4:
            ...
            self.write_buffers[sock] = struct.pack(">L", 0x80000000 | len(reply)) + reply
